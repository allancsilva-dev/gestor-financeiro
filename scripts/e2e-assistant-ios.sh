...
curl -fsS -X POST "$API/v1/onboarding/finalizar" "${AUTH[@]}" -H 'Content-Type: application/json' --data '
  {"carteira":{"nome":"Conta Principal","subtipo":"CORRENTE","saldo":1000,"banco":"E2E"},
   "cartao":null,"categorias":[{"nome":"Mercado","cor":"#123456","icone":"cart","valorEsperado":0}],
...
(
  cd "$MOBILE_DIR"
  EXPO_PUBLIC_API_BASE_URL="$API" APP_ENV=local-e2e APP_RELEASE_SHA="$RUN_ID" FORCE_BUNDLING=1 \
  xcodebuild -workspace "$WORKSPACE" -scheme "$SCHEME" -configuration Debug -sdk iphonesimulator \
    ...
    CODE_SIGNING_ALLOWED=NO FORCE_BUNDLING=1 "EXTRA_PACKAGER_ARGS=--dev false" \
    EXPO_PUBLIC_API_BASE_URL="$API" RCT_METRO_PORT=8082 EX_DEV_CLIENT_NETWORK_INSPECTOR=0 \
...
prove_financial_state() {
  local label="$1" expected="$2" file="$ARTIFACT_DIR/proof-$label.json"
  local transactions operations confirmations movements divergences
  ...
  jq -cn --arg label "$label" --argjson expected "$expected" --argjson transactions "$transactions" \
    ...
    '{flow:$label,expected:$expected,transactions:$transactions,operations:$operations,confirmations:$confirmations,movements:$movements,divergences:$divergences}' >"$file"
  jq -e '.transactions == .expected and .operations == .expected and .confirmations == .expected
    and .movements == .expected and .divergences == 0' "$file" >/dev/null || fail "prova financeira $label divergente."
}
...
CONFLICT_STATUS="$(curl -sS -o /dev/null -w '%{http_code}' -X POST "$API/v1/assistant/messages" "${AUTH[@]}" \
  ...
  --data '{"text":"fixture 11 hoje","conversationId":null}')"
...
if mkdir -p "$DOWNLOADS" 2>/dev/null; then
  ...
  ) >"$ARTIFACT_DIR/importacao-mobile/maestro.log" 2>&1 || fail "flow importacao-mobile falhou."
else
...
if rg -n -i --glob '*.log' --glob '*.json' --glob '*.txt' \
  '(bearer[[:space:]]+[A-Za-z0-9._-]+|access[_-]?token|api[_-]?key|wa_id|551[0-9]{8,}|token-secreto)' "$ARTIFACT_DIR" \
