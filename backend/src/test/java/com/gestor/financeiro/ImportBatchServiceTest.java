...
class ImportBatchServiceTest {
    ...
    void createNormalizesInstitutionAndReplaysSameIdempotencyKey() {
        ImportBatch first = service.create(usuario.getId(), ImportFormat.CSV, " nubank ", HASH_A, "import:1");
        ImportBatch replay = service.create(usuario.getId(), ImportFormat.CSV, "NUBANK", HASH_A, "import:1");
    ...
    void reusedIdempotencyKeyWithDifferentContentConflicts() {
        ...
        assertThrows(FinancialConflictException.class,
                () -> service.create(usuario.getId(), ImportFormat.CSV, null, HASH_B, "import:2"));
    }

    @Test
    void lifecycleAllowsOnlyDeclaredTransitions() {
        ImportBatch batch = service.create(usuario.getId(), ImportFormat.OFX, "001", HASH_A, null);
    ...
    void failureRequiresBoundedCode() {
        ImportBatch batch = service.create(usuario.getId(), ImportFormat.CSV, null, HASH_A, null);
    ...
    void rejectsMalformedBoundaryValues() {
        assertThrows(BusinessException.class,
                () -> service.create(usuario.getId(), ImportFormat.CSV, null, "ABC", null));
        assertThrows(BusinessException.class,
                () -> service.create(usuario.getId(), ImportFormat.CSV, "../../bank", HASH_A, null));
        assertThrows(BusinessException.class,
                () -> service.create(usuario.getId(), ImportFormat.CSV, null, HASH_A, "contains space"));
    }
}
