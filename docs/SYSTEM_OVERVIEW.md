...
19b. **Recorrencia de cartao ja suporta periodicidade nao-mensal (RESOLVIDO 2026-09-02/04, BACKLOG-0120 fechado):** ate 2026-08-29 `ContaFixa.plusMonths(1)` era hard-coded. Migration `V72` (commit `3ee1601`) introduziu `FrequenciaRecorrencia` SEMANAL..ANUAL; migration `V73__ancora_para_toda_frequencia.sql` (working tree em 2026-09-04, nao commitada) soltou `data_ancora` para toda frequencia exceto MENSAL, permitindo escolher o mes do aniversario de uma anual/bimestral/trimestral/semestral. Ver item 29b abaixo.
...
19d. **Estouro de limite de cartao agora notifica o usuario (RESOLVIDO 2026-09-02, commit `6fa53c0`, BACKLOG-0125 fechado):** decisao do dono do produto foi lancar e avisar, nunca bloquear; o "lancar" ja existia desde PROB-0098, e o "avisar" foi entregue nesta entrega.
...
29b. **Ciclo de vida completo da assinatura de cartao entregue (2026-09-02 a 2026-09-04, continuacao de PROB-0098; BACKLOG-0120/0125 fechados nesta sessao):** tres frentes. (1) **Frequencia deixa de ser so mensal:** migration `V72` (commit `3ee1601`) adiciona `FrequenciaRecorrencia` SEMANAL/QUINZENAL/MENSAL/BIMESTRAL/TRIMESTRAL/SEMESTRAL/ANUAL; migration `V73__ancora_para_toda_frequencia.sql` (working tree em 2026-09-04, **nao commitada**) solta `data_ancora` (antes exclusiva de SEMANAL/QUINZENAL) para toda frequencia exceto MENSAL, permitindo fixar o mes do aniversario ("todo 15 de marco") em vez de a serie sair de "hoje". No mobile, `SeletorDeFrequencia` (grade `flexWrap`, sem scroll) substitui a lista horizontal de chips nas duas telas de cadastro. (2) **Tres defeitos do motor de recorrencia corrigidos** (working tree, nao commitados): `ContaFixaService.atualizar` antecipava o aniversario ao recalcular a serie mesmo quando so nome/valor mudavam (BUG-0106); `reativar` nao revalidava o destino (recorrencia "zumbi" quando o cartao era excluido) nem chamava `pularOcorrenciasJaExecutadas` (trava em 400 permanente ao reativar no mesmo dia de uma ocorrencia ja realizada) — BUG-0107; `pularOcorrenciasJaExecutadas` comparava so a data exata do proximo vencimento, deixando passar o caso de mudar a ancora e voltar a serie para um mes ja cobrado com dia diferente (BUG-0108). (3) **Dois bugs so apareceram depois de recompilar o backend**, que estava servindo build V72 antiga durante a primeira rodada de verificacao (achado operacional, PROB-0100): HTTP 500 ao editar qualquer assinatura de cartao por `Conta` stub do JSON pendurado numa entidade ja gerenciada, causando cascata de entidade destacada no auto-flush do Hibernate (BUG-0109); mudar so o dia de vencimento nao movia a proxima cobranca porque o valor "anterior" era lido depois do setter (BUG-0110). O `quality-reviewer` desta sessao tambem achou que `reativar` nao recusava uma conta que ja tinha **concluido** o ciclo (`ativo=false` + `PAGO` de ocorrencia unica, o mesmo `ativo=false` de um cancelamento) — corrigido com 422 dedicado (BUG-0111); distinguir de verdade "cancelada" de "concluida" ainda depende de inferencia (`recorrente`+`status`), nao de uma coluna de motivo (BACKLOG-0133). Na UI mobile (`app/(app)/more/contas-fixas.tsx`), a tela ganhou o ciclo de vida completo: abas Todas/Assinaturas/Canceladas, botao Cancelar no card (com aviso de que cobrancas ja lancadas na fatura permanecem), Reativar na aba Canceladas, rotulo "Concluida" sem oferecer Reativar, e o switch "Repete todo mes" (que era no-op na edicao, pois `atualizar` nunca chamava `setRecorrente`) foi escondido no modo edicao e renomeado para "Repete sempre" na criacao. Evidencia: backend 595 testes/0 falhas; mobile 50 suites/584 testes/0 falhas, lint e `tsc --noEmit` limpos; migration V73 verificada em PostgreSQL real (`gestor_financeiro_verif`, flyway em v73); runtime no simulador iPhone 17 Pro contra backend real (screenshots: 7 frequencias sem rolar, anual com ancora mostrando competencia de fatura correta, editar nome preservou aniversario, cancelar/reativar/aba Assinaturas funcionando). Nada desta entrega foi commitado ate o fim da sessao (working tree). Ver PROB-0099 (crash de cache de cartoes entre telas, ja commitado em `cec09af`), PROB-0100 (build desatualizada mascarou os bugs 4a/4b/4c) e BUG-0104 a BUG-0111 para o detalhamento completo.
...
20. **Chave de cache React Query compartilhada entre telas e um ponto fragil sem convencao central (RESOLVIDO pontualmente em 2026-09-03, PROB-0099/BUG-0105, commit `cec09af`):** duas telas usando a mesma `queryKey` (`['cartoes']`) com `queryFn` de formatos diferentes causava crash de cold start; a correcao unificou o formato so para essa chave especifica, com um teste de regressao dedicado (`cacheCartoesCompartilhado.test.tsx`) — nao ha hook central (`useCartoes`) nem lint/teste generico que impeca a mesma classe de erro numa chave diferente no futuro. **Risco de processo relacionado (PROB-0100, 2026-09-04):** nao ha checagem automatica de que o backend local em execucao corresponde ao working tree atual antes de uma rodada de verificacao em runtime; nesta sessao isso mascarou tres bugs reais (BUG-0109/0110/0111) ate o backend ser recompilado (BACKLOG-0134).
