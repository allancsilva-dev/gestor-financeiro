...
**Ultima atualizacao:** 2026-07-11 (PROB MEDIUM — rate limit distribuido, sessao mobile body-only, validação PostgreSQL real via Docker CLI e backup criptografado)
...
14. **Pagamento parcial de fatura bloqueado por design:** `pagarFatura` exige o valor exato da fatura (soma dos lancamentos); decisao consciente mantida na revisao de 2026-07-09 (nao implementado suporte a saldo remanescente/rollover). Ver BACKLOG-0049.
...
16. **Invariante centralizado de limite de cartao:** `Conta.valorGasto == soma dos lancamentos em faturas nao pagas` (nao apenas soma de compras — inclui ajustes e estornos). Helpers privados `criarLancamento`/`removerLancamentoDeFaturaAberta`/`ajustarLimiteUtilizado` em `FaturaService` sao o unico ponto que ajusta `valorGasto` para compras de cartao; `TransacaoService` deixou de chamar `contaService.adicionarGasto`/`removerGasto` para transacoes que sao compra de cartao (mantido apenas para contas que nao sao cartao de credito). `pagarFatura` continua liberando limite pelo total da fatura, sem alteracao de contrato.
...
18. **Badge de status/tipo de lancamento na fatura (mobile) diferenciado por cor:** desde 2026-07-09 (PROB-0046/PROB-0047, BUG-0028/BUG-0029), o badge de status da fatura em `mobile/app/(app)/more/faturas.tsx` usa cores semanticas distintas para os 4 estados (`ABERTA` = cor de marca/estado normal, `FECHADA` = warning, `VENCIDA` = danger, `PAGA` = success, antes binario PAGA/resto); e cada lancamento de fatura exibe um chip de tipo (`ESTORNO` verde, `AJUSTE` ambar) com o prefixo textual `"Ajuste: "`/`"Estorno: "` removido da descricao exibida (o badge assume esse papel). O frontend web (`frontend/src/pages/Faturas.tsx`) ainda nao tem essa paridade — ver BACKLOG-0057.
...
23. **`Conta.valorGasto` pode ficar temporariamente negativo:** desde 2026-07-09 (PROB-0044/BUG-0024), um lancamento `ESTORNO` maior que as compras em aberto no cartao pode deixar `valorGasto` negativo (credito) — comportamento intencional que autocorrige na proxima compra/pagamento, mas sem tratamento de UX dedicado (BACKLOG-0053).
24. **Fatura com apenas estorno (total <= 0) nao e pagavel:** `pagarFatura` nao trata o caso de uma fatura cujo total e zero ou negativo — o credito fica "preso" aguardando compras futuras na mesma fatura, sem rollover explicito para a fatura seguinte ou para a carteira (BACKLOG-0054).
25. **Redistribuicao de parcelas na edicao nao recalcula parcela cheia:** `ressincronizarCompraCartao` divide o valor restante apenas entre as parcelas ainda nao pagas — decisao consciente de simplicidade que pode gerar parcelas com valores diferentes do parcelamento original (BACKLOG-0055).
26. **Mensagem de erro de pagamento de fatura pode confundir por corrida (nao por pagamento parcial real):** `handlePagar` (mobile e web) envia `fatura.valorTotal` capturado no ultimo fetch; se o total real mudar no backend entre o fetch e o toque em "Pagar Fatura", a mensagem generica "Pagamento parcial de fatura ainda nao e suportado" aparece mesmo sem o usuario ter feito nenhum pagamento parcial (BACKLOG-0056).
27. **Badge de tipo de lancamento (ajuste/estorno) implementado so no mobile:** `frontend/src/pages/Faturas.tsx` (web) ainda exibe apenas a cor condicional do valor, sem chip de tipo nem remocao do prefixo textual da descricao — divergencia de paridade com o mobile (BACKLOG-0057).
